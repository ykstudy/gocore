...
func TestNewErrCode(t *testing.T) {
	if rc := NewErrCode(1, "msg"); rc == nil {
		t.Fatal("NewErrCode return nil")
	}
}

func TestDecode(t *testing.T) {
	rc := NewErrCode(1, "msg", errors.New("err"))
...
func TestError(t *testing.T) {
	rc := NewErrCode(1, "msg", errors.New("err"), errors.New("xx"))
	err := rc.Error()
	if err != "err; xx" {
		t.Fatal("Error error")
	}
...
func TestErrs(t *testing.T) {
	...
	rc = NewErrCode(1, "msg", errors.New("x"))
	errs = rc.Errs()
	if len(errs) != 1 {
		t.Fatal("Errs error")
	}
